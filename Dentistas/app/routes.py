...
def agenda():
    ...
    if form.validate_on_submit():
        ...
        room=Room.query.filter_by(id=form.rooms.data).first()
        doctor=Doctor.query.filter_by(id=form.doctores.data).first()
        paciente=User.query.filter_by(id=form.pacientes.data).first()
        servicio=Servicio.query.filter_by(id=form.servicios.data).first()
        #condicional si click en boton editar costo, costo=entrada_costo, otro costo= costo=servicio.servicioCosto
        ...
        if booleano:
            ...
            #return costo=precio_nuevo.data
        #else:
            #return costo=servicio.servicioCosto
        ...
        meeting=Meeting(title=form.title.data,roomId=room.id,doctorId=doctor.id,servicioId=servicio.id,bookerId=booker.id,date=form.date.data,startTime=form.startTime.data,endTime=endTime,duration=form.duration.data, costo=costo)
        db.session.add(meeting)
        ...
        flash('Booking success!')
        return redirect(url_for('index'))
    return render_template('agenda.html',title='Book Meeting',form=form)
...
def editar():
    ...
    if form.validate_on_submit():
        ...
        modificacion=Meeting(id=meeting.id, title=form.observaciones.data, roomId=form.roomId.data, doctorId=form.doctorId.data, servicioId=form.servicioId.data, date=form.date.data, startTime=form.startTime.data, endTime=form.startTime.data+1, costo=form.costo.data )
        db.session.add(modificacion)
        db.session.commit()
        flash(f'Cita {meeting.servicio} con {User.query.filter_by(meeting.pacienteId).username} Modificada correctamente! ')
        return redirect(url_for('index'))
